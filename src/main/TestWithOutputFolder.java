...
class ConfigWithOutputFolder {
    ...
    public List<TestTypeWithOutputFolder> getTestTypes() {
        return testTypes;
    }

    public void setTestTypes(final List<TestTypeWithOutputFolder> testTypes) {
        this.testTypes = testTypes;
    }
}

class TestTypeWithOutputFolder {
...
public final class TestWithOutputFolder {
    ...
    private static final String OUT_FOLDER = "out/result/";
    private static final File TEST_INPUTS_FOLDER = new File(CHECKER_RESOURCES_FOLDER + IN_FOLDER);
    ...
    private static ConfigWithOutputFolder loadConfig() {
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            return objectMapper.readValue(CONFIG_FILE, ConfigWithOutputFolder.class);
        } catch (IOException e) {
    ...
    private static void runTest(
        ...
        if (!outFile.exists()) {
            ...
            try {
                var actual = objectMapper.readTree(outFile);
    ...
    private static String[] createTestArgv(final File testFile) {
        ...
        listArgv.add(OUT_FOLDER + "out_" + testFile.getName());
    ...
    private static int testMaxScore(
        ...
        for (TestTypeWithOutputFolder testType : config.getTestTypes()) {
